...
class RunSimpleSimulation(SIRSimulation):
    ...
    def setup(self):
        ...
        for city in range(self.n_cities):
            ...
            self.city_plot[city+1].set_ylim(bottom=0,top=self.box_size)
            self.group_plot[city+1] = dict()
            self.group_plot[city+1]["S"] = self.city_plot[city+1].scatter(self.particles[city+1]["S"][:,0],self.particles[city+1]["S"][:,1], color = "blue")
            self.group_plot[city+1]["I"] = self.city_plot[city+1].scatter(self.particles[city+1]["I"][:,0],self.particles[city+1]["I"][:,1], color = "red")
            self.group_plot[city+1]["A"] = self.city_plot[city+1].scatter(self.particles[city+1]["A"][:,0],self.particles[city+1]["A"][:,1], color = "yellow")
            self.group_plot[city+1]["R"] = self.city_plot[city+1].scatter(self.particles[city+1]["R"][:,0],self.particles[city+1]["R"][:,1], color = "grey")
        ...
        self.time_label = tex.annotate(self.boxes[1][1].time,xy=(0.8,0.76),fontsize=14)
        tex.annotate("Days:",xy=(0.1,0.5),fontsize=14)
        self.Days = tex.annotate(int(self.boxes[1][1].time/0.2),xy=(0.8,0.5),fontsize=14)
        tex.annotate("R:",xy=(0.1,0.16),fontsize=14)
        self.R_label = tex.annotate(self.effect_reproduction_num,xy=(0.8,0.16),fontsize=14)
